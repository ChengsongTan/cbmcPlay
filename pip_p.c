...
# 1 "los_atomic.h" 1
# 36 "los_atomic.h"
# 1 "los_arch_atomic.h" 1
# 44 "los_arch_atomic.h"
...
# 18 "pip.c" 2
 ...
 static void M(void){
     ...
     int j = 0;
     UINT32 numticks = nondet_int();
     UINT32 innerloop = nondet_int();
     __CPROVER_assume(innerloop < 1 && numticks < 1);
     ...
     do { printf("M finished with j = %d, took %d ticks", j, tickNum); } while (0);
...
# 84 "pip.c"
     ...
     osEndTime = LOS_TickCountGet();
     tickNum = (osEndTime - osStartTime);
     do { printf("H got lock m in %d ticks\n", tickNum); } while (0);
     muxPended = (((LosMuxCB *)g_allMux) + (m));
     printf("H %d\n", muxPended->owner->taskID);
     printf("H %s\n", muxPended->owner->taskName);
     LOS_MuxPost(m);
     ...
     if (tickNum > nondet_int()) {
         do { printf("H took too long to get its lock!!!!!!!! tickNum = %d\n", tickNum); } while (0);
     }
 ...
 static UINT32 TestCase(void)
 {
  ...
  __CPROVER_assert(0, "create of lock n failed\n");
     ...
     task1.pcName = "H";
     task1.usTaskPrio = 5;
     task1.uwResved = 0x0100;
     g_testCount = 0;

     do { if ((ret) != (0U)) { ICunitSaveErr(133, (iiUINT32)ret); return 1; } } while (0);
     ...
     task2.pcName = "Hp";
     task2.usTaskPrio = 6;
     task2.uwResved = 0x0100;
     g_testCount = 0;


     do { if ((ret) != (0U)) { ICunitSaveErr(144, (iiUINT32)ret); return 1; } } while (0);
     ...
     task3.pcName = "M";
     task3.usTaskPrio = 10;
     task3.uwResved = 0x0100;
     g_testCount = 0;

     do { if ((ret) != (0U)) { ICunitSaveErr(154, (iiUINT32)ret); return 1; } } while (0);
     ...
     do { if ((ret) != (0U)) { ICunitSaveErr(171, (iiUINT32)ret); return 1; } } while (0);
 ...
 void main(void)
 {
    do { iUINT32 uwRet = 1; uwRet = ICunitAddCase("ItLosAtomic001", (CASE_FUNCTION)TestCase, TEST_LOS, TEST_ATO, TEST_LEVEL0, TEST_FUNCTION); do { if ((uwRet) != (0x00000000)) { ICunitSaveErr(180, (iiUINT32)uwRet); return; } } while (0); } while (0);
    __CPROVER_assert(0, "smoke test early term");
    TestCase();
    iUINT32 uwRet = 1;


    __CPROVER_assert(uwRet == 0x00000000, "smoke test");
    TestCase();
    return 1;
 }
