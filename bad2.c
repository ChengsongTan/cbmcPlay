...
//   while(assertion) {//do many runs until assertion fails
    ...
    will_acquire[3][0] = abs(nondet_int() % MAX_EXECUTION_TIME);
    locking_time[3][0] = abs(nondet_int() % MAX_EXECUTION_TIME);
    will_acquire[3][1] = abs(nondet_int() % MAX_EXECUTION_TIME);
    locking_time[3][1] = abs(nondet_int() % MAX_EXECUTION_TIME);

    will_acquire[2][0] = abs(nondet_int() % MAX_EXECUTION_TIME);
    will_acquire[2][1] = abs(nondet_int() % MAX_EXECUTION_TIME);
    locking_time[2][0] = abs(nondet_int() % MAX_EXECUTION_TIME);
    locking_time[2][1] = abs(nondet_int() % MAX_EXECUTION_TIME);

    running_time[2] = abs(nondet_int() % MAX_EXECUTION_TIME);

    will_acquire[1][0] = abs(nondet_int() % MAX_EXECUTION_TIME);
    locking_time[1][0] = abs(nondet_int() % MAX_EXECUTION_TIME);
    will_acquire[1][1] = abs(nondet_int() % MAX_EXECUTION_TIME);
    locking_time[1][1] = abs(nondet_int() % MAX_EXECUTION_TIME);

    will_acquire[0][0] = abs(nondet_int() % MAX_EXECUTION_TIME);
    locking_time[0][0] = abs(nondet_int() % MAX_EXECUTION_TIME);
    will_acquire[0][1] = abs(nondet_int() % MAX_EXECUTION_TIME);
    locking_time[0][1] = abs(nondet_int() % MAX_EXECUTION_TIME);
        ...
        for (int j = 0; j < NUM_LOCKS; j++) {//now check each lock, and try to release/acquire as necessary
            if (mutex[j] == to_run ){//if thread is holding the lock
                ...
                if (locking_time[to_run][j] <= 0) {
                    ...
                    //comment this if block here for an incorrect implementation: switch to the original priority
