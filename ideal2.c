...
int main()
{
    ...
    for (int i = 0; i < NUM_THREADS; i++){
        ...
        blocked[i] = -1;
        waiting_time[i] = 0;
        sleeping[i][0] = 0;//abs(nondet_int() % (running_time[i]));//this value indicates thread i should try to sleep after time sleeping[i][0]
        sleeping[i][1] = NUM_THREADS - i;//abs(nondet_int() % (running_time[i] - sleeping[i][0]));//indicates thread i should in total sleep for sleeping[i][1]
