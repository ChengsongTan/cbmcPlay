...
//   while(assertion) {//do many runs until assertion fails
    ...
    will_acquire[3][0] = abs(nondet_int() % 16);
    locking_time[3][0] = abs(nondet_int() % 16);
    will_acquire[3][1] = abs(nondet_int() % 16);
    locking_time[3][1] = abs(nondet_int() % 16);

    will_acquire[2][0] = abs(nondet_int() % 16);
    will_acquire[2][1] = abs(nondet_int() % 16);
    locking_time[2][0] = abs(nondet_int() % 16);
    locking_time[2][1] = abs(nondet_int() % 16);

    running_time[2] = abs(nondet_int() % 16);

    will_acquire[1][0] = abs(nondet_int() % 16);
    locking_time[1][0] = abs(nondet_int() % 16);
    will_acquire[1][1] = abs(nondet_int() % 16);
    locking_time[1][1] = abs(nondet_int() % 16);

    will_acquire[0][0] = abs(nondet_int() % 16);
    locking_time[0][0] = abs(nondet_int() % 16);
    will_acquire[0][1] = abs(nondet_int() % 16);
    locking_time[0][1] = abs(nondet_int() % 16);
        ...
        for (int j = 0; j < NUM_LOCKS; j++) {//now check each lock, and try to release/acquire as necessary
            if (mutex[j] == to_run ){//if thread is holding the lock
                ...
                if (locking_time[to_run][j] <= 0) {
                    ...
                    //the commented out block here is the correct implementation: it should switch to the highest remaining priority, instead of its original prio
